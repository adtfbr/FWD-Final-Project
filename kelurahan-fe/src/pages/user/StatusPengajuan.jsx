    ...
    <div className="p-0">
      ...
      {submissions.length === 0 ? (
        ...
        <div className="space-y-4">
          {submissions.map((sub) => (
            <SubmissionCard key={sub.id_pengajuan || sub.id} sub={sub} />
          ))}
        </div>
      )}
    </div>
...
const SubmissionCard = ({ sub }) => (
  <div className="bg-white p-6 shadow-md rounded-lg flex flex-col lg:flex-row gap-6 transition-all hover:shadow-lg">
      ...
      {sub.status === 'Selesai' ? (
        sub.file_surat_hasil ? (
          <a
            ...
            className="btn bg-green-600 hover:bg-green-700 text-white w-full shadow-sm border-none flex items-center justify-center gap-2 normal-case text-sm h-10 min-h-0 transition-transform active:scale-95"
          >
