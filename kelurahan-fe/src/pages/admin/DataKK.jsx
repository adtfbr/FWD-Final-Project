...
// (REVISI FINAL: +Search Bar & SweetAlert Integrasi)
          ...
          <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto animate-fade-in-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={resetFormAndClose} className="px-5 py-2.5 bg-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-300 transition-colors">Batal</button>
                <button type="submit" className="px-5 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 shadow-md transition-all" disabled={submitLoading}>
                  {submitLoading ? <span className="loading loading-spinner loading-sm"></span> : "Simpan Data"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-5xl rounded-xl shadow-2xl p-0 overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
            ...
            <div className="bg-blue-600 p-4 flex justify-between items-center text-white">
              ...
              <button onClick={closeDetailModal} className="text-white/80 hover:text-white text-2xl">&times;</button>
            </div>
                ...
                <>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-bold text-lg text-gray-800">{selectedKK.no_kk}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Kepala Keluarga</p>
                      <p className="font-bold text-lg text-gray-800">{selectedKK.nama_kepala_keluarga}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Alamat</p>
                      <p className="font-bold text-gray-800">{selectedKK.alamat}, RT {selectedKK.rt}/RW {selectedKK.rw}</p>
                    </div>
                  </div>
...
function Input({ label, name, value, handle, type = "text", required = false, placeholder = '' }) {
  return (
    <div className="w-full">
      ...
      <input type={type} name={name} value={value || ''} onChange={handle} placeholder={placeholder} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-gray-50 focus:bg-white" required={required} />
    </div>
  );
}
...
function TextArea({ label, name, value, handle, required = false }) {
  return (
    <div className="w-full">
      <label className="block text-sm font-medium text-gray-700 mb-1.5">{label} {required && <span className="text-red-500">*</span>}</label>
      <textarea name={name} value={value || ''} onChange={handle} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-gray-50 focus:bg-white" required={required} rows={3} />
    </div>
  );
}