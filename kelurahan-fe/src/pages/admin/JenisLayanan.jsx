              ...
              filteredLayanan.map((layanan, index) => (
                <tr key={layanan.id_jenis_layanan || layanan.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3 flex justify-center gap-2">
                    <ActionButton onClick={() => handleOpenModal(layanan)} color="bg-yellow-500 hover:bg-yellow-600" icon={<FaEdit />} />
                    <ActionButton onClick={() => handleDelete(layanan.id_jenis_layanan || layanan.id)} color="bg-red-600 hover:bg-red-700" icon={<FaTrash />} />
                  </td>
                </tr>
              ))
            )}
...
const LayananFormModal = ({ isOpen, onClose, isEdit, formData, handleInputChange, handleSubmit, loading }) => {
...
const TextArea = ({ label, name, value, handle, required = false, placeholder = "" }) => (
  <div className="w-full">
    <label className="block text-sm font-medium text-gray-700 mb-1.5">
      {label} {required && <span className="text-red-500">*</span>}
    </label>
