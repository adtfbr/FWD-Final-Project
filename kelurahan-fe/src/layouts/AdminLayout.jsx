      ...
      >
        ...
        <div className="px-4 pb-4 flex-1 overflow-y-auto scrollbar-hide">
          ...
          <ul className="space-y-1">
            <NavItem to="/admin/dashboard" icon={<FaTachometerAlt />} onClick={() => setIsSidebarOpen(false)}>Dashboard</NavItem>
            <NavItem to="/admin/pengajuan-surat" icon={<FaFileAlt />} onClick={() => setIsSidebarOpen(false)}>Pengajuan Surat</NavItem>
            <NavItem to="/admin/verifikasi-warga" icon={<FaCheckCircle />} onClick={() => setIsSidebarOpen(false)}>Verifikasi Warga</NavItem>
          </ul>

          <p className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-2 pl-3 mt-6">Data Master</p>
          <ul className="space-y-1">
            <NavItem to="/admin/penduduk" icon={<FaUsers />} onClick={() => setIsSidebarOpen(false)}>Data Penduduk</NavItem>
            <NavItem to="/admin/kk" icon={<FaIdCard />} onClick={() => setIsSidebarOpen(false)}>Data Kartu Keluarga</NavItem>
            <NavItem to="/admin/jenis-layanan" icon={<FaListAlt />} onClick={() => setIsSidebarOpen(false)}>Jenis Layanan</NavItem>
          </ul>

          <p className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-2 pl-3 mt-6">Informasi & Laporan</p>
          <ul className="space-y-1">
            <NavItem to="/admin/berita" icon={<FaNewspaper />} onClick={() => setIsSidebarOpen(false)}>Berita Desa</NavItem>
            <NavItem to="/admin/laporan" icon={<FaChartBar />} onClick={() => setIsSidebarOpen(false)}>Laporan Statistik</NavItem>
          </ul>
        </div>
